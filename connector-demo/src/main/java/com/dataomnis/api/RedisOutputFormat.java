...
import com.dataomnis.api.util.RedisConnectionUtils;
import com.dataomnis.api.util.RedisValueType;
...
public class RedisOutputFormat<T> extends RichOutputFormat<T> {
    ...
    public RedisOutputFormat(String redisIp, int redisPort, String redisPassword, String redisValueType, int ttl) {
        this.redisIp = redisIp;
        this.redisPort = redisPort;
        this.redisPassword = redisPassword;
        this.ttl = ttl;
        this.type = RedisValueType.getRedisValueType(redisValueType);
    }
        ...
        switch (type) {
            case REDIS_VALUE_TYPE_STRING:
                ...
                for (int i = 0; i < record.getArity(); i++) {
                    sb.append(record.getField(i));
                    if (i != record.getArity() - 1) {
                        sb.append(",");
                    }
                }
                writeStringValue(redisKey, sb.toString());
                break;
    ...
    private void delete(RowData record) {
        final String redisKey = record.getString(0).toString();
    ...
    private void insert(RowData record) {
        ...
        switch (type) {
            case REDIS_VALUE_TYPE_STRING:
                StringBuilder sb = new StringBuilder();
                for (int i = 0; i < record.getArity(); i++) {
                    sb.append(record.getString(i).toString());
                    if (i!=record.getArity()-1){
                        sb.append(",");
                    }
                }
            ...
            case REDIS_VALUE_TYPE_HASH:
                ...
                for (int i = 0; i < keyArray.size(); i++) {
                    hashValue.put(keyArray.getString(i).toString(), valueArray.getString(i).toString());
                }
                writeHashValue(redisKey, hashValue);
                break;
            case REDIS_VALUE_TYPE_LIST:
                ...
                break;
            case REDIS_VALUE_TYPE_SET:
                final ArrayData arraySet = record.getArray(1);
                ...
                break;
            default:
                throw new RuntimeException(String.format("unknown redis value type [%s]", type));
        }
    }
